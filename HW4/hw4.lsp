...
(defun node2var (n c k)
    (+ (* (- n 1) k) c)   ; return variable index based off formula
)
...
(defun at-least-one-color (n c k)
    (cond
        ((> c k) nil)
        (t (append (list (node2var n c k)) (at-least-one-color n (+ c 1) k)))   ; append current variable index to rest
    )
)

; helper function for at-most-one-color
(defun at-most-helper (n c x k)
    (cond
        ((> x k) nil)
        (t (append (list (list (- (node2var n c k)) (- (node2var n x k)))) (at-most-helper n c (+ x 1) k)))
    )
)
...
(defun at-most-one-color (n c k)
    (cond 
        ((= c k) nil)   ; empty set
        (t (append (at-most-helper n c 1 k) (at-most-one-color n (+ c 1) k)))   
    )
)
...
(defun generate-node-clauses (n k)
    (cons (at-least-one-color n 1 k) (at-most-one-color n 1 k))   ; append at least and at most one color with c set to 1
)
...
(defun generate-edge-clauses (e k)
    (generate-edge-clauses-helper (first e) (second e) 1 k)   ; call the helper function 
)
...
(graph-coloring-to-sat "graph1.txt" "output1.txt" 3)